...
public class Snake {
	...
	private int lengthBeforeHeadOn = 5;
	...
	public boolean selfCollision(Snake snake) {
		for (int i = 1; i < this.length; i++) {
			...
			// checks head on collision condition and sets length to 0 for each snake
				...
				this.setLengthBeforeHeadon(this.getLength());
				this.setLength(0);
				this.setHeadOn(true);
				return false;
			}
	...
	private void setHeadOn(boolean truthVal) {
		this.headOn = truthVal;
	}

	public boolean checkHeadOn() {
		return this.headOn;
	}

	private void setLengthBeforeHeadon(int length) {
		this.lengthBeforeHeadOn = length;
	}

	public int getLengthBeforeHeadOn() {
		return this.lengthBeforeHeadOn;
	}
	...
	public void setDirection(char a) {
		...
		if (!((a == 'L' && this.direction == 'R') || (a == 'R' && this.direction == 'L')
			...
			// these conditions check for the cases, where snake cannot make 180 degree
			// turns, such as right to left
