...
public class GamePanel extends JPanel implements ActionListener {
	...
	BufferedImage[] snakeImages = new BufferedImage[2];
	...
	GamePanel(int playerNumberStart) {
		...
		try {
			snakeImages[0] = ImageIO.read(new File("../images/snake_body.jpeg"));
		} catch (IOException e) {
		}

		try {
			snakeImages[1] = ImageIO.read(new File("../images/snake_body2.jpeg"));
		} catch (IOException e) {
		}
			...
			for (int j = 0; j < playerNumber; j++) {
				...
				for (int i = 0; i < snakes[j].getLength(); i++) {
					...
					// i != 0 --> this is the snake's body
						...
						g.drawImage(snakeImages[j], snakes[j].xPos[i], snakes[j].yPos[i], snakes[j].xPos[i] + tile_size,
								snakes[j].yPos[i] + tile_size, 0, 0, snakeImages[j].getWidth(),
								snakeImages[j].getHeight(), null);
					}
				}
			}
		...
		for (int r = 0; r < playerNumber; r++) {
			if (playerNumberStart == 1) {
				if (running) {
					...
					scoreText = "" + snakes[r].getLengthBeforeHeadOn();
				}
			} else {
